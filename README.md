...
- Due to repeating errors in using the program, development is stopped/slowed. May be required to wait for advancement of, my own hardware or model technologies/varietes or updating relevant files from main branch. I assume the problem is between LiteLLM and Ollama, though this could be the models too, here making simple maze game with 4 files (see EXAMPLES section)...
```
